-- Migration: 20260202_fix_create_invoice_locked_return_query.sql
-- Fix ambiguous "id" by removing bare id reference and using inv.* with alias
...
BEGIN
  ...
  -- Insert invoice (no ambiguous id usage)
  ...
  -- Return inserted row SAFELY using alias inv.*
  RETURN (
    SELECT inv.*
    FROM public.invoices inv
    WHERE inv.invoice_no = v_invoice_no
