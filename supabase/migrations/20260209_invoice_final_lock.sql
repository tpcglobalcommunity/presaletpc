-- Migration: 20260209_invoice_final_lock.sql
-- Description: Final DB lock for invoice flow with enforcement
...
-- ================================================================
-- PHASE 1 — ENUM STATUS + SAFE MIGRATION
-- ================================================================
...
-- Drop existing default on status column if exists
ALTER TABLE public.invoices ALTER COLUMN status DROP DEFAULT;

-- Normalize existing statuses to enum
UPDATE public.invoices 
SET status = CASE 
    WHEN LOWER(status) = 'unpaid' THEN 'UNPAID'::invoice_status
    WHEN LOWER(status) = 'pending' THEN 'PENDING_REVIEW'::invoice_status
    WHEN LOWER(status) = 'pending_review' THEN 'PENDING_REVIEW'::invoice_status
    ...
    WHEN status IS NULL OR status = '' THEN 'UNPAID'::invoice_status
    ELSE 'UNPAID'::invoice_status
END;

-- Convert status column to enum safely
DO $$ BEGIN
    BEGIN
        ALTER TABLE public.invoices ALTER COLUMN status TYPE invoice_status USING status::text::invoice_status;
    EXCEPTION
        WHEN others THEN
            -- Column already converted or other issue, continue
            NULL;
    END;
END $$;
...
-- ================================================================
-- PHASE 2 — REQUIRED COLUMNS (ADD IF NOT EXISTS)
-- ================================================================
...
-- ================================================================
-- PHASE 3 — DATA INTEGRITY CONSTRAINTS (DROP & RECREATE)
-- ================================================================

-- Drop existing constraints if they exist
DO $$ BEGIN
    ALTER TABLE public.invoices DROP CONSTRAINT IF EXISTS chk_pending_review_requires_proof;
EXCEPTION
...
DO $$ BEGIN
    ALTER TABLE public.invoices DROP CONSTRAINT IF EXISTS chk_approved_requires_admin;
EXCEPTION
    WHEN undefined_object THEN
        NULL;
END $$;
...
DO $$ BEGIN
    ALTER TABLE public.invoices DROP CONSTRAINT IF EXISTS chk_paid_requires_tx;
EXCEPTION
    WHEN undefined_object THEN
        NULL;
END $$;
...
DO $$ BEGIN
    ALTER TABLE public.invoices DROP CONSTRAINT IF EXISTS chk_wallet_address_required;
EXCEPTION
    WHEN undefined_object THEN
        NULL;
END $$;
...
ADD CONSTRAINT chk_pending_review_requires_proof
CHECK (
...
ALTER TABLE public.invoices 
ADD CONSTRAINT chk_approved_requires_admin
CHECK (
...
ALTER TABLE public.invoices 
ADD CONSTRAINT chk_paid_requires_tx
CHECK (
...
ALTER TABLE public.invoices 
ADD CONSTRAINT chk_wallet_address_required
CHECK (
...
-- ================================================================
-- PHASE 4 — STATUS TRANSITION GUARD (TRIGGER HARD LOCK)
-- ================================================================
...
-- Create trigger
DROP TRIGGER IF EXISTS public.trg_invoice_status_guard ON public.invoices;
CREATE TRIGGER public.trg_invoice_status_guard
...
-- ================================================================
-- PHASE 5 — RLS FINAL LOCK (MEMBER vs ADMIN)
-- ================================================================
...
DO $$ BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'is_admin') THEN
        ...
        AS $$
        BEGIN
            RETURN FALSE; -- Safe stub, should be replaced with actual implementation
        END;
        $$;
    END IF;
END $$;

-- Create member RPC for proof submission
...
BEGIN
    ...
    -- Get current invoice status and ownership
    ...
    -- Update invoice with proof and set to pending review
    UPDATE public.invoices
    SET 
...
-- Create admin RPC for rejection
...
-- Create admin RPC for marking as paid
...
CREATE POLICY "Users can submit proof via RPC only" ON public.invoices
    FOR UPDATE USING (
        user_id = auth.uid() 
