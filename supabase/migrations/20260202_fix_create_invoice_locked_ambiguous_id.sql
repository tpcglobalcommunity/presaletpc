-- Fix create_invoice_locked ambiguous id + add SOL price stale check
-- Migration: 20260202_fix_create_invoice_locked_ambiguous_id.sql
...
BEGIN
  ...
  ELSIF v_currency = 'SOL' THEN
    ...
    -- Read SOL price + updated_at from cache
    ...
    -- Stale check: reject if older than 10 minutes
...
-- Ensure frontend can call it
GRANT EXECUTE ON FUNCTION public.create_invoice_locked(text, text, text, numeric) TO anon, authenticated;
