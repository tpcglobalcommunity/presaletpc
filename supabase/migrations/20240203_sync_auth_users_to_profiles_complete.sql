-- =====================================================
-- Sync Auth Users to Profiles for Admin Display
-- =====================================================
-- Ensures email and full_name are available in public.profiles
-- Fixes Admin Users page display issues

-- =====================================================
-- PHASE 1 — ENSURE PROFILES SCHEMA
-- =====================================================
...
-- Sync existing data from auth.users to public.profiles
...
DECLARE
    ...
    email_count integer;
    full_name_count integer;
BEGIN
    -- Count statistics
    SELECT COUNT(*) INTO total_profiles FROM public.profiles;
    SELECT COUNT(*) INTO email_count FROM public.profiles WHERE email IS NOT NULL AND email != '';
    SELECT COUNT(*) INTO full_name_count FROM public.profiles WHERE full_name IS NOT NULL AND full_name != '';
    
    RAISE NOTICE 'Backfill Results:';
    RAISE NOTICE 'Total profiles: %', total_profiles;
    RAISE NOTICE 'Profiles with email: %', email_count;
    RAISE NOTICE 'Profiles with full_name: %', full_name_count;
END $$;

-- =====================================================
-- PHASE 3 — AUTO SYNC NEW USERS
-- =====================================================
...
BEGIN
    ...
    SELECT COUNT(*) INTO final_null_name_count FROM public.profiles WHERE full_name IS NULL OR full_name = '';
    
    RAISE NOTICE '=====================================================';
    RAISE NOTICE 'Auth Users to Profiles Sync Summary';
    RAISE NOTICE '=====================================================';
    ...
    RAISE NOTICE 'Profiles without full_name: %', final_null_name_count;
    RAISE NOTICE '=====================================================';
    RAISE NOTICE 'Expected Results:';
    RAISE NOTICE '- Admin Users page displays email correctly';
    RAISE NOTICE '- Admin Users page displays full_name correctly';
    RAISE NOTICE '- New Google login users auto-populate';
