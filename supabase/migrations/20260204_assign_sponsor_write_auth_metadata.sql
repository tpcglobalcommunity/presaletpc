-- =========================================================
-- assign_sponsor: also write sponsor to auth.user_metadata
-- so it is visible in Supabase Auth Users panel
-- =========================================================
...
begin
  ...
  if v_existing is not null then
    -- ensure auth metadata is also present (idempotent)
    update auth.users
    set raw_user_meta_data =
      coalesce(raw_user_meta_data, '{}'::jsonb)
      || jsonb_build_object('sponsor_code', v_existing_code, 'sponsor_user_id', v_existing::text)
    where id = v_uid;
  ...
  if v_ref is not null then
    ...
    if v_ref_sponsor_id is not null then
      ...
      -- write to auth metadata (visible in Auth Users)
      update auth.users
      set raw_user_meta_data =
        coalesce(raw_user_meta_data, '{}'::jsonb)
        || jsonb_build_object('sponsor_code', v_ref_sponsor_code, 'sponsor_user_id', v_ref_sponsor_id::text)
      where id = v_uid;
  ...
  set raw_user_meta_data =
    coalesce(raw_user_meta_data, '{}'::jsonb)
    || jsonb_build_object('sponsor_code', v_best_code, 'sponsor_user_id', v_best_id::text)
  where id = v_uid;
