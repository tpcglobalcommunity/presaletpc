-- =====================================================
-- ULTIMATE COMPLETE SQL SETUP FOR ADMIN PAGES (ALL ISSUES FIXED)
-- Copy and paste this entire script in Supabase SQL Editor
-- This script fixes ALL constraint issues
-- =====================================================
...
-- 2. TEMPORARILY DISABLE CONSTRAINT (OPTIONAL - ONLY IF NEEDED)
-- Uncomment ini jika constraint masih error:
-- ALTER TABLE profiles ALTER COLUMN user_id DROP NOT NULL;
-- ALTER TABLE profiles ALTER COLUMN user_id SET DEFAULT gen_random_uuid();

-- 3. CREATE RPC FUNCTION FOR ADMIN USERS (VERIFIED)
...
-- 4. CREATE RPC FUNCTION FOR DASHBOARD STATS (VERIFIED)
...
-- 8. INSERT SAMPLE INVOICES (FIXED - dengan valid user_id)
...
(gen_random_uuid(), 'TPC-2024-0004', 'sarah.johnson@example.com', '550e8400-e29b-41d4-a716-4466554404', 'IDR', 5100000, 5100000, 'PAID', NOW() - INTERVAL '15 days', NOW() - INTERVAL '10 days', NOW() - INTERVAL '10 days'),
...
(gen_random_uuid(), 'TPC-2024-0008', 'lisa.garcia@example.com', '550e8400-e29b-41d4-a716-4466554408', 'IDR', 6800000, 6800000, 'PAID', NOW() - INTERVAL '6 days', NOW() - INTERVAL '1 day', NOW() - INTERVAL '1 day'),

-- Unpaid invoices (4)
(gen_random_uuid(), 'TPC-2024-0009', 'robert.martinez@example.com', '550e8400-e29b-41d4-a716-4466554409', 'IDR', 1700000, 1700000, 'UNPAID', NOW() - INTERVAL '4 days', NOW() + INTERVAL '2 days', NOW()),
(gen_random_uuid(), 'TPC-2024-0010', 'jennifer.anderson@example.com', '550e8400-e29b-41d4-a716-44665544010', 'IDR', 3400000, 3400000, 'UNPAID', NOW() - INTERVAL '3 days', NOW() + INTERVAL '3 days', NOW()),
(gen_random_uuid(), 'TPC-2024-0011', 'william.taylor@example.com', '550e8400-e29b-41d4-a716-4466554011', 'IDR', 1700000, 1700000, 'UNPAID', NOW() - INTERVAL '2 days', NOW() + INTERVAL '4 days', NOW()),
(gen_random_uuid(), 'TPC-2024-0012', 'mary.thomas@example.com', '550e8400-e29b-41d4-a716-44665544012', 'IDR', 5100000, 5100000, 'UNPAID', NOW() - INTERVAL '1 day', NOW() + INTERVAL '5 days', NOW()),

-- Pending review invoices (3)
(gen_random_uuid(), 'TPC-2024-0013', 'james.jackson@example.com', '550e8400-e29b-41d4-a716-44665544013', 'IDR', 1700000, 1700000, 'PENDING_REVIEW', NOW() - INTERVAL '12 hours', NOW() + INTERVAL '6 days', NOW() - INTERVAL '6 hours'),
(gen_random_uuid(), 'TPC-2024-0014', 'patricia.white@example.com', '550e8400-e29b-41d4-a716-44665544014', 'IDR', 3400000, 3400000, 'PENDING_REVIEW', NOW() - INTERVAL '6 hours', NOW() + INTERVAL '5 days', NOW() - INTERVAL '3 hours'),
(gen_random_uuid(), 'TPC-2024-0015', 'richard.harris@example.com', '550e8400-e29b-41d4-a716-44665544015', 'IDR', 1700000, 1700000, 'PENDING_REVIEW', NOW() - INTERVAL '2 hours', NOW() + INTERVAL '6 days', NOW() - INTERVAL '1 hour')
ON CONFLICT (id) DO NOTHING;
...
LEFT JOIN (
    SELECT 
        ...
        COUNT(CASE WHEN status = 'PAID' THEN 1 END) as paid_invoices,
        COUNT(CASE WHEN status = 'UNPAID' THEN 1 END) as unpaid_invoices,
        COALESCECE(SUM(CASE WHEN status = 'PAID' THEN amount_input ELSE 0 END), 0) as total_revenue
    FROM invoices
    GROUP BY user_id
