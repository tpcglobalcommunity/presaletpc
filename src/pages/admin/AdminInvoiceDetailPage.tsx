// Admin Invoice Detail Page - Fixed: 2025-02-05 13:38 UTC
...
export default function AdminInvoiceDetailPage() {
  ...
  console.debug('[ADMIN_INVOICE_DETAIL_REAL] mounted', { invoiceKey });
  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      ...
      console.debug('[ADMIN_INVOICE_DETAIL_REAL] mode-detect', { invoiceKey });

      try {
        setIsLoading(true);
        
        const mode = isUUID(invoiceKey) ? 'id' : 'invoice_no';
        console.debug('[ADMIN_INVOICE_DETAIL_REAL] will query', { mode, invoiceKey });
        ...
        console.debug('[ADMIN_INVOICE_DETAIL_REAL] primary result', { data, error });

        if (error) {
          console.error('[ADMIN_INVOICE_DETAIL_REAL] primary error:', error);
          // Try fallback
          const fallbackMode = mode === 'id' ? 'invoice_no' : 'id';
          console.debug('[ADMIN_INVOICE_DETAIL_REAL] fallback start', { fallbackMode });
          ...
          console.debug('[ADMIN_INVOICE_DETAIL_REAL] fallback result', { data: fallbackData, error: fallbackError });
          ...
          // Try fallback
          const fallbackMode = mode === 'id' ? 'invoice_no' : 'id';
          console.debug('[ADMIN_INVOICE_DETAIL_REAL] primary null, fallback start', { fallbackMode });
        ...
        console.error('[ADMIN_INVOICE_DETAIL_REAL] fetch error:', error);
        setNotFound(true);
      } finally {
        setIsLoading(false);
      }
