...
export default function OnboardingPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.nama.trim()) errors.push('Name is required');
    if (!formData.email.trim()) errors.push('Email is required');
    if (!formData.phone.trim()) errors.push('Phone number is required');
    if (!formData.city.trim()) errors.push('City is required');
    if (!formData.wallet_address.trim()) errors.push('Wallet address is required');
    if (formData.wallet_address.trim().length < 32 || formData.wallet_address.trim().length > 44) {
      errors.push('Wallet address must be 32-44 characters');
    }
