...
export default function CompleteProfilePage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.no_wa.trim()) errors.push('No WA wajib diisi');
    if (!isValidWhatsAppNumber(formData.no_wa)) errors.push('No WA tidak valid (min 8 digit, hanya angka dan +)');
    if (!formData.kota.trim()) errors.push('Kota wajib diisi');
    
    const walletField = getWalletFieldName();
    if (!formData[walletField].trim()) {
      errors.push('Alamat wallet TPC wajib diisi');
    } else if (!isValidWalletAddress(formData[walletField])) {
      errors.push('Alamat wallet TPC harus 32-44 karakter');
    }
